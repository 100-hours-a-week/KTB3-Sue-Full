...
public class ProxyTest {
    ...
    void proxyTest_Hibernate() {
        // 데이터 준비
        User user = new User("ProxyTester@adapterz.kr", "123aS!", "tester", UserRole.ADMIN);
        ...
        System.out.println("작성자 이메일 = " + authorProxy.getEmail()); // 접근 시 초기화
    }
    ...
    void proxyTest_OriginJPA() {
        // 데이터 준비
        User user = new User("ProxyTester2@adapterz.kr", "123aS!", "tester", UserRole.ADMIN);
        entityManager.persist(user);

        Post post = new Post("프록시 테스트2 - JPA version", "내용", PostType.FREE, user);
        entityManager.persist(post);
        ...
        String initializeUserEmail = authorProxy.getEmail();
        System.out.println("실제 DB SELECT을 통해 엔티티 데이터 로딩");
        System.out.println("작성자 이메일 = " + initializeUserEmail); // 접근 시 초기화
    }
